...
exports.addProductToCart = catchAsync(async (req, res, next) => {
  ...
  if (quantity > product.quantity) {
    return next(
      new AppError(400, `Only these products left ${product.quantity} :( `)
    );
  }
...
exports.updateCartProduct = catchAsync(async (req, res, next) => {
  ...
  // If quantity is 0, mark the product's and change status as removed
  if (quantity === 0) {
    await productInCart.update({ quantity: 0, status: 'removed' });
  }

  // Update product to new quantifyty
  if (quantity > 0) {
